#!/usr/bin/env python3
"""
KubixAgent -- Export optimized kernels to HuggingFace Kernels format.

Takes an optimized KubixAgent CUDA or Triton kernel and packages it into the
...
Usage:
    ...
    uv run export_hf.py --name my_matmul --repo-id kubix-ai/matmul-kernel
    ...
    #   kernels upload . --repo_id kubix-ai/matmul-kernel
...
def generate_init_py(
    ...
    if backend == "cuda":
        return textwrap.dedent(f'''\
            """
            {name} - Optimized GPU kernel exported from KubixAgent
            https://github.com/Kubix-AI/kubix_agent
        ...
        return textwrap.dedent(f'''\
            """
            {name} - Optimized Triton GPU kernel exported from KubixAgent
            https://github.com/Kubix-AI/kubix_agent
...
def _export_triton_kernel(
    ...
    pyproject_content = textwrap.dedent(f"""\
        ...
        name = "{name}"
        version = "0.1.0"
        description = "Optimized Triton GPU kernel exported from KubixAgent"
    ...
    readme_content = textwrap.dedent(f"""\
        ...
        Optimized Triton GPU kernel exported from [KubixAgent](https://github.com/Kubix-AI/kubix_agent).
...
def export_kernel(
    ...
    ----------
    kernel_path : str
        Path to the KubixAgent kernel file (kernel.py or similar).
    name : str
    ...
    repo_id : str, optional
        HuggingFace repo ID (e.g., "kubix-ai/matmul-kernel").
    ...
    print(f"=== KubixAgent HuggingFace Kernels Export ===")
    print()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Export an optimized KubixAgent kernel to HuggingFace Kernels format. "
        ...
        epilog=textwrap.dedent("""\
            examples:
              ...
              uv run export_hf.py --name my_matmul --kernel workspace/kernel_matmul_1.py \\
                                  --repo-id kubix-ai/matmul-kernel
